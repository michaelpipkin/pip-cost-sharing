  ...
  private readonly helpSections: HelpSection[] = [
    ...
    },
    {
      ...
      content: [
        ...
        "If the rental isn't shared evenly - a master suite that should cost more than a shared bedroom or a couch, for example - turn on \"Rooms have different rates.\" Add a room for each space, give it a rate relative to a standard room (1.0), and assign participants to it. A room's rate is split between however many of its occupants are present each night, so two people sharing a room each pay less than someone with a room to themselves. Anyone not assigned to a room is charged the standard rate. Leave the toggle off for the original even split.",
