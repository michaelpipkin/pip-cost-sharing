...
describe('GroupService', () => {
  ...
  describe('constructor - restoring saved group from localStorage', () => {
    it('should set current group from localStorage on initialization', () => {
      localStorage.setItem('currentGroup', JSON.stringify({ id: 'group-1', name: 'Test Group' }));
  ...
  describe('addGroup', () => {
    ...
    it('should create group, member, and default category in a batch', async () => {
      const result = await service.addGroup({ name: 'New Group' }, { displayName: 'Alice' });
    ...
    it('should always create a category named "Default"', async () => {
      ...
      expect(categorySetCall![1]).toMatchObject({ name: 'Default', active: true });
    });
  });
  ...
  describe('updateGroup', () => {
    ...
    it('should update the group document', async () => {
      ...
      expect(mockBatch.update).toHaveBeenCalledWith(groupRef, { name: 'New Name' });
    });

    it('should clear defaultGroupRef for all users when deactivating', async () => {
      const groupRef = { id: 'group-1' } as any;
      const userDoc = { ref: { id: 'user-1' } };
      vi.spyOn(firestoreModule, 'getDocs').mockResolvedValueOnce(makeSnap([userDoc]) as any);

      await service.updateGroup(groupRef, { active: false });

      expect(mockBatch.update).toHaveBeenCalledWith(userDoc.ref, { defaultGroupRef: null });
    });
  ...
  describe('deleteGroup', () => {
    ...
    it('should call the deleteGroup cloud function', async () => {
      const mockDeleteFn = vi.fn().mockResolvedValue({ data: { success: true } });
      vi.spyOn(functionsModule, 'httpsCallable').mockReturnValue(mockDeleteFn as any);

      await service.deleteGroup('group-1');

      expect(functionsModule.httpsCallable).toHaveBeenCalledWith(mockFunctions, 'deleteGroup');
      expect(mockDeleteFn).toHaveBeenCalledWith({ groupId: 'group-1' });
    });

    it('should throw when cloud function returns success: false', async () => {
      const mockDeleteFn = vi.fn().mockResolvedValue({ data: { success: false, message: 'Permission denied' } });
      vi.spyOn(functionsModule, 'httpsCallable').mockReturnValue(mockDeleteFn as any);

      await expect(service.deleteGroup('group-1')).rejects.toThrow('Permission denied');
    });
  ...
  describe('getGroup', () => {
    ...
    it('should throw when the group document does not exist', async () => {
      ...
      await expect(service.getGroup({ id: 'ghost-group' } as any, {} as any)).rejects.toThrow(
    ...
    it('should initialize all group-related services', async () => {
      ...
      expect(mockMemorizedService.getMemorizedExpensesForGroup).toHaveBeenCalled();
