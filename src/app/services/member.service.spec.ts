...
describe('MemberService', () => {
  ...
  beforeEach(() => {
    ...
    vi.spyOn(firestoreModule, 'addDoc').mockResolvedValue({ id: 'new-member' } as any);
  ...
  describe('addMemberToGroup', () => {
    it('should throw when a member with the same email already exists', async () => {
      vi.spyOn(firestoreModule, 'getDocs')
        .mockResolvedValueOnce(makeSnap([makeDocSnap('m1', { email: 'dup@test.com' })]) as any);

      await expect(
        service.addMemberToGroup('group-1', { email: 'dup@test.com' }),
      ).rejects.toThrow('A member with this email address already exists in the group.');
    });
    ...
    it('should link userRef when a matching user account exists', async () => {
      ...
      vi.spyOn(firestoreModule, 'getDocs')
        ...
        .mockResolvedValueOnce(makeSnap([makeDocSnap('user-123', {}, mockUserRef)]) as any); // user found
      vi.spyOn(firestoreModule, 'addDoc').mockResolvedValueOnce({ id: 'new-member' } as any);
  ...
  describe('updateMember', () => {
    ...
    it('should throw when another member has the same email', async () => {
      vi.spyOn(firestoreModule, 'getDocs').mockResolvedValueOnce(makeSnap([makeDocSnap('other-member', {})]) as any);
    ...
    it('should call updateDoc when the email is unique', async () => {
      ...
      expect(firestoreModule.updateDoc).toHaveBeenCalledWith(mockMemberRef, { email: 'new@test.com' });
    });
    ...
    it('should skip duplicate email check when email is not being changed', async () => {
      ...
      expect(firestoreModule.updateDoc).toHaveBeenCalledWith(mockMemberRef, { displayName: 'Alice' });
    });
  });
  ...
  describe('updateMemberWithUserMatching', () => {
    ...
    it('should search for a user and link userRef when member is unlinked and email is changing', async () => {
      ...
      vi.spyOn(firestoreModule, 'getDocs').mockResolvedValueOnce(makeSnap([makeDocSnap('user-abc', {}, mockUserRef)]) as any);
      vi.spyOn(firestoreModule, 'updateDoc').mockResolvedValueOnce(undefined);

      const changes: any = { email: 'new@test.com' };
      await service.updateMemberWithUserMatching(mockMemberRef, changes, null, 'old@test.com');

      expect(changes.userRef).toBe(mockUserRef);
      expect(firestoreModule.updateDoc).toHaveBeenCalledWith(mockMemberRef, changes);
    });
    ...
    it('should not search when member already has a userRef', async () => {
      ...
      const changes: any = { email: 'new@test.com' };
      await service.updateMemberWithUserMatching(mockMemberRef, changes, existingRef, 'old@test.com');
    ...
    it('should not search when email is not changing', async () => {
      ...
      await service.updateMemberWithUserMatching(mockMemberRef, changes, null, 'same@test.com');
  ...
  describe('removeMemberFromGroup', () => {
    const mockMemberRef = { id: 'member-1', eq: (other: any) => other.id === 'member-1' } as any;
    ...
    it('should throw when the member has existing splits', async () => {
      ...
      vi.spyOn(firestoreModule, 'getDocs').mockResolvedValueOnce(makeSnap([splitDoc]) as any);

      await expect(service.removeMemberFromGroup('group-1', mockMemberRef)).rejects.toThrow(
  ...
  describe('leaveGroup', () => {
    ...
    it('should throw when the member is the only admin', async () => {
      vi.spyOn(firestoreModule, 'getDocs')
        .mockResolvedValueOnce(makeSnap([makeDocSnap('member-1', { groupAdmin: true })]) as any);

      await expect(service.leaveGroup('group-1', mockMemberRef)).rejects.toThrow(
        'You are the only group admin.',
      );
    });
    ...
    it('should mark the member inactive when they have splits', async () => {
      ...
      vi.spyOn(firestoreModule, 'getDocs')
        .mockResolvedValueOnce(makeSnap([makeDocSnap('member-1', {}), makeDocSnap('member-2', {})]) as any)
      ...
      expect(firestoreModule.updateDoc).toHaveBeenCalledWith(mockMemberRef, { active: false });
    });
  ...
  describe('updateAllMemberEmails', () => {
    ...
    it('should update email on all member documents linked to the user', async () => {
      ...
      vi.spyOn(firestoreModule, 'getDocs').mockResolvedValueOnce(makeSnap(docs) as any);
      vi.spyOn(firestoreModule, 'updateDoc').mockResolvedValue(undefined);

      const count = await service.updateAllMemberEmails(mockUserRef, 'new@test.com');
    ...
    it('should rethrow errors', async () => {
      ...
      await expect(service.updateAllMemberEmails(mockUserRef, 'new@test.com')).rejects.toThrow('Firestore error');
    });
  });
});
