...
function makeSnap(size: number) {
  return { size, empty: size === 0, docs: Array.from({ length: size }, (_, i) => ({ id: `doc-${i}`, data: () => ({}) })) };
}

describe('CategoryService', () => {
  let service: CategoryService;
  const mockCategoryStore = { setGroupCategories: vi.fn(), groupCategories: signal<any[]>([]) };
  const mockAnalytics = { logEvent: vi.fn().mockResolvedValue(undefined) };

  beforeEach(() => {
    vi.clearAllMocks();
    vi.spyOn(firestoreModule, 'collection').mockReturnValue(mockCollection as any);
  ...
  describe('addCategory', () => {
    it('should throw when a category with the same name already exists', async () => {
      vi.spyOn(firestoreModule, 'getDocs').mockResolvedValueOnce(makeSnap(1) as any);

      await expect(service.addCategory('group-1', { name: 'Groceries' })).rejects.toThrow(
        'This category already exists.',
      );
    });

    it('should call addDoc when the category name is unique', async () => {
      vi.spyOn(firestoreModule, 'getDocs').mockResolvedValueOnce(makeSnap(0) as any);
      vi.spyOn(firestoreModule, 'addDoc').mockResolvedValueOnce({ id: 'new-cat' } as any);
    ...
    it('should return the new document reference', async () => {
      ...
      const result = await service.addCategory('group-1', { name: 'Groceries' });
  ...
  describe('updateCategory', () => {
    ...
    it('should call updateDoc when the name is unique', async () => {
      ...
      expect(firestoreModule.updateDoc).toHaveBeenCalledWith(mockCategoryRef, { name: 'New Name' });
    });

    it('should not call updateDoc when name is duplicate', async () => {
      vi.spyOn(firestoreModule, 'getDocs').mockResolvedValueOnce(makeSnap(1) as any);

      await expect(service.updateCategory(mockCategoryRef, { name: 'Groceries' })).rejects.toThrow();
