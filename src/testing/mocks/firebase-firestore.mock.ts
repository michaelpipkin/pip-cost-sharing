...
export class Timestamp {
  ...
  static fromDate = vi.fn().mockImplementation(
    (d: Date) => new Timestamp(Math.floor(d.getTime() / 1000), 0),
  );
  static now = vi.fn().mockImplementation(() => new Timestamp(Math.floor(Date.now() / 1000), 0));
...
export const collection: (db: any, path: string, ...pathSegments: string[]) => CollectionReference<unknown> =
  vi.fn().mockReturnValue({}) as any;

export const collectionGroup: (db: any, collectionId: string) => Query<unknown> =
  vi.fn().mockReturnValue({}) as any;

export const doc: (...args: any[]) => DocumentReference<unknown> =
  vi.fn().mockReturnValue(new DocumentReference()) as any;

export const documentId: () => FieldPath = vi.fn().mockReturnValue({}) as any;

export const query: (queryOrRef: any, ...constraints: QueryConstraint[]) => Query<unknown> =
  vi.fn().mockReturnValue({}) as any;

export const where: (field: string | FieldPath, op: WhereFilterOp, value: any) => QueryConstraint =
  vi.fn().mockReturnValue({}) as any;

export const orderBy: (field: string | FieldPath, direction?: OrderByDirection) => QueryConstraint =
  vi.fn().mockReturnValue({}) as any;

export const limit: (n: number) => QueryConstraint = vi.fn().mockReturnValue({}) as any;
...
export const getDocs: (query: Query<any> | CollectionReference<any>) => Promise<QuerySnapshot<any>> =
  vi.fn().mockResolvedValue({ docs: [], size: 0, empty: true, forEach: vi.fn() }) as any;

export const getDoc: (ref: DocumentReference<any>) => Promise<DocumentSnapshot<any>> =
  vi.fn().mockResolvedValue({ id: '', exists: () => false, data: () => undefined }) as any;

export const addDoc: (ref: CollectionReference<any>, data: any) => Promise<DocumentReference<any>> =
  vi.fn().mockResolvedValue(new DocumentReference('mock-id')) as any;

export const updateDoc: (ref: DocumentReference<any>, data: any) => Promise<void> =
  vi.fn().mockResolvedValue(undefined) as any;

export const deleteDoc: (ref: DocumentReference<any>) => Promise<void> =
  vi.fn().mockResolvedValue(undefined) as any;

export const setDoc: (ref: DocumentReference<any>, data: any, options?: any) => Promise<void> =
  vi.fn().mockResolvedValue(undefined) as any;
